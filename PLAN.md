...
8. ⚠️ Обработка nullable reference types - **НЕ РЕАЛИЗОВАНО** (nullable reference types должны помечаться явно через `?`)
...
- Обработка nullable reference types (должны помечаться явно через `?`)
