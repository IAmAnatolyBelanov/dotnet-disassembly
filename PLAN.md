...
6. Извлечение атрибутов (включаются в генерируемый код)
...
5. Генерация атрибутов для типов и членов
6. Добавление XML-комментариев как тривиальных комментариев
7. Обработка дублирующихся имен классов через счетчик (первое появление без индекса, последующие с индексом)
...
**Задачи:**
1. ~~Обработка вложенных типов (nested types)~~ Решено: переиспользование счетчика для именования файлов
2. Обработка partial классов
3. ~~Обработка атрибутов (включить или исключить?)~~ Решено: атрибуты включаются в генерируемый код
